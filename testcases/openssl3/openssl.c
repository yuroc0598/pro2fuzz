...
//#endif
	...
	do{
		...
		if (r == -1) {
			if ((SSL_get_error(client, r) != SSL_ERROR_WANT_WRITE) && (SSL_get_error(client, r) != SSL_ERROR_WANT_READ)){
				err();
				break;
			}
		}
		...
		write_packet(TPstate,buf,r);
	
		if (TPstate == pstate) {
#ifdef __AFL_HAVE_MANUAL_CONTROL
			__AFL_INIT();
#endif
			if(res_shm) pstate = shmptr[MAP_SIZE];
			if(pstate == TPstate){
				f = fopen(ifi, "rb");
			...
			else r= read_packet(TPstate,buf);
		} 		
		...
		if (r == -1) {
			if ((SSL_get_error(server, r) != SSL_ERROR_WANT_WRITE) && (SSL_get_error(server, r) !=	SSL_ERROR_WANT_READ)){err();break;}
		}
